...
* 集計くんPro_10.exe を右クリックし、「プロパティ」を選択します。「全般」タブの下部にある「セキュリティ」または「全般」セクションで「このコンピュータを保護するため、このファイルへのアクセスはブロックされる可能性があります」の横にある「許可する」にチェックを入れ、「OK」をクリックしてください。
...
### 8. To Do
* 転記処理への対応
* キーワード.json、キーワードの同義語.json の GUI化
* バッチ処理への対応
* 複数の処理の保存機能の追加
