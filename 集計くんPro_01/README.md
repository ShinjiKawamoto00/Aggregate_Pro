...
- [3. フォルダー構成](#3-フォルダー構成)
- [4. インストールと実行](#4-インストールと実行)
    - [4.1 リリース物の配置](#41-リリース物の配置)
    - [4.2 実行](#42-実行)
...
集計くんPro フォルダーを Windows PC 上の任意の場所に配置してください。

#### 4.2 実行

Windows PowerShell で集計くんProフォルダーに移動して下記コマンドで実行してください。なお、集計くんPro を実行する際、集計先ファイルや転記先ファイルをエクセルで開いていると動作は保証されません。必ずこれらのファイルを閉じてから実行してください。
...
[3. フォルダー構成](#3-フォルダー構成) に記述の通り、入力JSON は datasets フォルダ配下に設置し、ファイル場所の指定、処理の指定などを行います。
